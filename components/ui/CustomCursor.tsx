    ...
    <>
      ...
      <motion.div
        ...
        style={{
          ...
          opacity: visible ? 1 : 0,
          scale: hovered ? 0 : 1,
          transition: "scale 0.25s ease, opacity 0.3s ease",
        }}
      />

      {/* Ring */}
      <motion.div
        className="fixed top-0 left-0 z-[9998] pointer-events-none rounded-full border border-black flex items-center justify-center overflow-hidden"
        style={{
          x: ringX,
          y: ringY,
          translateX: `-${ringOffset}px`,
          translateY: `-${ringOffset}px`,
          width: ringSize,
          height: ringSize,
          opacity: visible ? 1 : 0,
          transition:
            "width 0.4s cubic-bezier(0.16,1,0.3,1), height 0.4s cubic-bezier(0.16,1,0.3,1), opacity 0.3s ease",
        }}
      >
        {label && (
          <span
